...
public:
    ResourceManager(const std::string& filePath) : dataFilePath(filePath), nextCpuId(1), nextGpuId(1) {}
    ...
    // 获取所有资源
    const std::vector<Resource*>& getAllResources() const { return resources; }
    ...
    std::string generateUniqueResourceId(ResourceType type) {
        ...
        int& nextId = (type == ResourceType::CPU) ? nextCpuId : nextGpuId;
        
        // 生成ID并递增计数器
        std::string id = prefix + std::string(3 - std::to_string(nextId).length(), '0') + std::to_string(nextId);
        nextId++;
