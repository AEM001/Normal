...
public:
    ...
    bool registerUser(const std::string& username, const std::string& password, UserRole role) {
        ...
        switch (role) {
            case UserRole::STUDENT:
                newUser = std::make_unique<Student>(userId, username, password);
                break;
            case UserRole::TEACHER:
                newUser = std::make_unique<Teacher>(userId, username, password);
                break;
            case UserRole::ADMIN:
                newUser = std::make_unique<Admin>(userId, username, password);
                break;
            default:
                return false; // 无效的角色
        }
